...
def main():
    ...
    rhodelta_file = os.path.join(rhodelta_map_dir, 'K50_deltarhomap_s25_50rpk.npz')
    ...
    for i, rho in enumerate(rhos):
        d = data_KofS[rho]
        ax1.errorbar(d['Ss'], d['K50s'], yerr=d['K50_errs'], fmt='o--', capsize=3, label=(fr'$\rho = -⅓$') if rho==-1./3. else fr'$\rho = {rho}$', color=rhocols[i])
    ...
    for i, rho in enumerate(rhos):
        d = data_Kofdelta[rho]
        ax2.errorbar(d['deltas'], d['K50s'], yerr=d['K50_errs'], fmt='o--', capsize=3, label=(fr'$\rho = -⅓$') if rho==-1./3. else fr'$\rho = {rho}$', color=rhocols[i])
